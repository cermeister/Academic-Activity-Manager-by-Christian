...
function App() {
  ...
  return <div className={`app ${mobileNavOpen ? "navOpen" : ""}`}>
    ...
    <main>
      ...
      {selected === "dashboard" && <><section className="stats"><Stat icon={<BookOpen/>} label="Subjects" value={stats.subjects}/><Stat icon={<FileText/>} label="Requirements" value={stats.total}/><Stat icon={<Clock3/>} label="Pending" value={stats.pending}/><Stat icon={<CheckCircle2/>} label="Completed" value={stats.completed}/><Stat icon={<CircleAlert/>} label="Overdue" value={stats.overdue}/></section><section className="dashboardGrid"><div className="panel"><div className="panelHead"><div><h2>Upcoming deadlines</h2><span>Your next requirements</span></div><CalendarDays size={20}/></div><div className="list">{[...allItems].filter(item => item.status !== "Completed").sort((a, b) => a.deadline.localeCompare(b.deadline)).map(item => <ItemRow key={item.id} x={item} onStatusChange={updateStatus} onDelete={remove} onEdit={() => setModal({type: "item", item})}/>)}</div>{allItems.filter(item => item.status !== "Completed").length === 0 && <Empty text="Everything is completed. Great work!"/>}</div><div className="panel"><div className="panelHead"><div><h2>Subjects</h2><span>Quick overview</span></div></div>{subjects.map(subject => <SubjectCard key={subject.id} subject={subject} items={items} onSelect={setSelected}/>)}</div></section></>}
...
function SubjectView({items, onStatusChange, onDelete, onEdit}) { const [filter, setFilter] = useState("All"); const shown = items.filter(item => filter === "All" || item.category === filter || item.status === filter).sort((a, b) => a.deadline.localeCompare(b.deadline)); return <section className="panel subjectPanel"><div className="filters">{["All", "Module", "Task", "Activity", "Others", "Pending", "Ongoing", "Completed"].map(value => <button className={filter === value ? "sel" : ""} onClick={() => setFilter(value)} key={value}>{value}</button>)}</div><div className="list">{shown.map(item => <ItemRow key={item.id} x={item} onStatusChange={onStatusChange} onDelete={onDelete} onEdit={() => onEdit(item)}/>)}</div>{!shown.length && <Empty text="No requirements found."/>}</section>; }
