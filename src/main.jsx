...
const sortByDeadlineAndPriority = (a, b) => {
  if (!a.deadline && !b.deadline) return (priorityOrder[a.priority] ?? 1) - (priorityOrder[b.priority] ?? 1);
  if (!a.deadline) return 1;
  if (!b.deadline) return -1;
  return a.deadline.localeCompare(b.deadline) || (priorityOrder[a.priority] ?? 1) - (priorityOrder[b.priority] ?? 1);
};
...
function App() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      const [sectionResult, subjectResult, itemResult, profileResult] = await Promise.all([
        ...
        supabase.from("requirements").select("*").eq("account_username", workspaceUsername).order("deadline", {ascending: true, nullsFirst: false}),
  ...
  return <div className={`app ${mobileNavOpen ? "navOpen" : ""} ${isViewer ? "viewerMode" : ""}`}>
    ...
    {modal && (modal.type === "subject" ? <SubjectModal subject={modal.subject} onSave={saveSubject} onDelete={deleteSubject} close={() => setModal(null)}/> : <RequirementModal data={modal} subjects={subjects} onSaveItem={saveItem} close={() => setModal(null)}/>)}
...
function RequirementModal({data, subjects, onSaveItem, close}) {
  const old = data.item;
  const [form, setForm] = useState(old ? {...old, deadline: old.deadline || "", files: old.files || []} : {subjectId: data.subjectId || subjects[0]?.id, category: "Task", title: "", description: "", deadline: "", priority: "Medium", status: "Pending", files: []});
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  const save = async () => {
...
function formatDate(date) { if (!date) return "No deadline"; return new Date(`${date}T00:00:00`).toLocaleDateString(undefined, {month: "short", day: "numeric", year: "numeric"}); }
