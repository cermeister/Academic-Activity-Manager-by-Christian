...
function StatusMenu({status, onChange}) {
  ...
  return <label className={`statusMenu ${status.toLowerCase()}`}><CheckCircle2 size={16}/><select className="statusButton" value={status} onChange={event => onChange(event.target.value)} aria-label={`Change status from ${status}`}>{options.map(option => <option key={option} value={option}>{option}</option>)}</select><ChevronRight className="statusChevron" size={14}/></label>;
}
