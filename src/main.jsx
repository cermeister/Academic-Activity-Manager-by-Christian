...
function AccessPanel({managedAccount, onSelect, onClose}) {
  ...
  return <div className="overlay"><div className="modal accessPanel"><div className="modalHead"><div><h2>Account access</h2><span>Choose the account you want to manage.</span></div><button onClick={onClose} aria-label="Close account access"><X/></button></div><div className="accountAccessList">{availableAccounts.map(account => <div key={account.username} className={`accessAccount ${selectedAccount === account.username ? "selected" : ""}`}><div><b>{account.username}</b><small>{account.role}</small><span>{account.workspace}</span></div><button type="button" className="primary" onClick={() => {setSelectedAccount(account.username); onSelect(account.username);}}>{managedAccount === account.username ? "Open current workspace" : "Access Workspace"}</button></div>)}</div><div className="accessNote"><LockKeyhole size={18}/><p>Workspace ownership remains with the selected account. Admin access is temporary and does not transfer or overwrite that workspace's records.</p></div><div className="modalActions"><button onClick={onClose}>Cancel</button><button className="primary" onClick={() => onSelect(selectedAccount)}>Open workspace</button></div></div></div>;
}
