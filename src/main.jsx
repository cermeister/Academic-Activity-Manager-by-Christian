...
function SubjectView({items, onStatusChange, onDelete, onEdit}) { const [filter, setFilter] = useState("All"); const shown = items.filter(item => filter === "All" || item.category === filter || item.status === filter).sort((a, b) => a.deadline.localeCompare(b.deadline)); return <section className="panel subjectPanel"><div className="filters">{["All", "Module", "Task", "Activity", "Others", "Pending", "Ongoing", "Completed"].map(value => <button className={filter === value ? "sel" : ""} onClick={() => setFilter(value)} key={value}>{value}</button>)}</div><div className="list">{shown.map(item => <LegacyItemRow key={item.id} x={item} onStatusChange={onStatusChange} onDelete={onDelete} onEdit={() => onEdit(item)}/>)}</div>{!shown.length && <Empty text="No requirements found."/>}</section>; }
...
function StatusMenu({status, onChange}) {
  ...
  return <div className="statusMenu">
    <button type="button" className={`statusButton ${status.toLowerCase()}`} onClick={() => setOpen(value => !value)} aria-expanded={open} aria-haspopup="listbox"><CheckCircle2 size={16}/>{status}<ChevronRight className={open ? "statusChevron open" : "statusChevron"} size={14}/></button>
    {open && <div className="statusOptions" role="listbox">{options.map(option => <button type="button" role="option" aria-selected={option === status} className={option === status ? "selected" : ""} key={option} onClick={() => {setOpen(false); if (option !== status) onChange(option);}}>{option}</button>)}</div>}
  </div>;
}
