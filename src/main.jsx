...
function DescriptionText({text}) { const [expanded, setExpanded] = useState(false); const description = text || "No description."; const isLong = description.length > 150; const visibleText = !expanded && isLong ? `${description.slice(0, 150).trimEnd()}...` : description; return <p className={expanded ? "descriptionExpanded" : ""}>{visibleText}{isLong && <button type="button" className="seeMoreButton" onClick={() => setExpanded(value => !value)}>{expanded ? " See less" : " See more"}</button>}</p>; }
